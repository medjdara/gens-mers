...
$pdf->MultiCell(0, 10, "
 تشهد الحكومة الجزائرية الديمقراطية و الشعبية أن الشهادة رقم  ".$row['num_brevet']."  أصدرت في ".$row['date_delivrance']."  إلى السيد/ة ".$row['nom_marin_arabe']." المولود  في ".$row['lieu_de_naissance_arabe']." بتاريخ ".$row['date_de_naissance']." مؤهل طبقا لأحكام اللائحة  ".$row['nom_regularisation']." من الإتفاقية المذكورة أعلاه ، في صيغتها المعدلة و أنه يتمتع بالكفاءة لأداء الوظائف التالية في المستويات المحددة ، طبقا لأية قيود مشار إليها حتى ".$row['date_expiration']."
", 0, '');
$pdf->setRTL(false);
$pdf->SetXY(13,66);
$pdf->SetFont('freeserif', '', 12, '', true);
$pdf->MultiCell(0, 10, "
The Government of People's Democratic Republic of Algeria certifies that Certificate N° ".$row['num_brevet']." was issued in ".$row['date_delivrance']." to Mr/s ".$row['nom_marin'].", born in (".$row['lieu_de_naissance']."), on ".$row['date_de_naissance']." has been found duly qualified in accordance with the provisions of regulation ".$row['nom_regularisation']." of the above Convention, as amended, and has been found competent to perform the following functions, at the levels specified, subject to any limitations indicated until ".$row['date_expiration'].".
", 0, '');
//-------table 1 ---------------
